...
@PerFragment class TagsPresenter @Inject constructor() {
    ...
    @Inject lateinit var databaseModel: RxTagsDatabaseModel
    ...
    private fun searchDatabase(): Observable.Transformer<CharSequence, QueryResult> {
        return Observable.Transformer { charSequenceObservable ->
            ...
            sequenceObservable
                    ...
                    .flatMap({ query ->
                        databaseModel.getAllFiltered(query)
        ...
        subscriptions.add(
                view.newTagDialog(viewModel.editTagDialogTitle, tag.displayName)
                        .flatMap { newName ->
                            ...
                            databaseModel.updateRefresh(tag)
                        }
                        .map { cursor -> InsertResult(cursor, databaseModel.findInCursor(cursor, tag)) }
                        .observeOn(AndroidSchedulers.mainThread())
    ...
    private fun checkIfUsed(tag: Tag, holder: TagItemHolder) : Observable<Tag> {
        ...
        if (tag.ingredientTypes.isEmpty())
            return databaseModel.getById(tagId)
                    .observeOn(AndroidSchedulers.mainThread())
